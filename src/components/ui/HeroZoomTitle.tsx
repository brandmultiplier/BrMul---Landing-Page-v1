...
export default function HeroZoomTitle({ titlePhrases = [], subtitle = "" }: HeroZoomTitleProps) {
    ...
    const Content = ({ variant }: { variant: 'base' | 'spotlight' }) => (
        <div className="flex flex-col gap-0 sm:gap-1 md:gap-2 w-full">
            <h1 className="text-[clamp(2.8rem,9vw,9.5rem)] leading-[0.9] font-medium tracking-tighter">
            ...
            <h2 className={`text-[clamp(1.2rem,4.5vw,7rem)] leading-[0.9] tracking-tighter font-light transition-colors duration-200 ${variant === 'spotlight' ? "text-white drop-shadow-[0_2px_10px_rgba(255,255,255,0.3)]" : "text-white/40"
                }`}>
