...
export default function StoryLockTaxCalculator({
  gated = true,
}: {
  gated?: boolean;
}) {
  ...
  const reportOpen = !gated || unlocked;
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined") {
        if (
          gated &&
          window.localStorage.getItem(UNLOCK_STORAGE_KEY) === "1"
        ) {
          setUnlocked(true);
        }
  ...
  const handleTabClick = (next: TabKey): void => {
    if (!reportOpen && (next === "levels" || next === "proof")) {
      ...
      <div className="sltax-wrap">
        ...
        {tab === "calc" && (
          <div>
            ...
            <div style={{ position: "relative" }}>
              <div
                style={{
                  filter: reportOpen ? "none" : "blur(6px)",
                  pointerEvents: reportOpen ? "auto" : "none",
                  userSelect: reportOpen ? "auto" : "none",
                }}
              >
                <BreakdownTable
                  hire={hireTax}
        ...
        {tab === "levels" && (
          <div>
            ...
            {LEVELS.map((lv) => {
              const current = reportOpen && lv.n === verdict.levelMatch;
              return (
                <div
                  key={lv.n}
                  style={{
